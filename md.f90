MODULE MD
    ...
    FUNCTION LJPOT(R, I) RESULT(LJP)
        ...
        REAL, DIMENSION(N - 1) :: DR, LJP ! DISTANCE BETWEEN NTH ATOM AND ITH ATOM
        INTEGER :: ROW, NEW_ROW, I

        NEW_ROW = 1

        DO ROW = 1, N ! CALCULATE COMPONENT WISE DISTANCE VECTORS. EX: IF ATOM K HAS POSITION VECTOR (1, 2, 3) AND ATOM L HAS POSITION VECTOR (5, 4, 2) THEN THE RESULTING VECTOR IS (4, 2, -1). THIS VECTOR CAN THEN BE USED FOR EUCLIDEAN DISTANCE CALCULATION.
            DRV(ROW, :) = R(ROW, :) - R(I, :)
        END DO

        DO ROW = 1, N 
            IF (ROW.EQ.I) THEN ! EXCLUDE ITH ATOM
                CYCLE
            END IF
            NDRV(NEW_ROW, :) = DRV(ROW, :)
            NEW_ROW = NEW_ROW + 1
        END DO

        DO ROW = 1, N - 1 ! CALCULATE EUCLIDEAN DISTANCE FOR EACH ATOM
            DR(ROW) = SQRT()
        END DO
